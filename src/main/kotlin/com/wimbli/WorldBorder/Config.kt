...
object Config {
    ...
    private fun parseMessage(msg: String): Component = if (msg.contains('<') && msg.contains('>')) {
    ...
    fun availableMemory(): Int = ((rt.maxMemory() - rt.totalMemory() + rt.freeMemory()) / 1048576L).toInt() // bytes in 1 MB
    ...
    fun replaceAmpColors(message: String): String = LegacyComponentSerializer.legacySection().serialize(
        LegacyComponentSerializer.legacyAmpersand().deserialize(message),
    )

    // strip all color/formatting codes, via Adventure
    fun stripAmpColors(message: String): String = PlainTextComponentSerializer.plainText().serialize(
        LegacyComponentSerializer.legacyAmpersand().deserialize(message),
    )
        ...
        if (worlds != null) {
            for (worldNameRaw in worlds.getKeys(false)) {
                ...
                val border = BorderData(
                    bord.getDouble("x", 0.0),
                    bord.getDouble("z", 0.0),
    ...
    fun save(logIt: Boolean, storeFillTask: Boolean = false) {
        ...
        // self-documenting config: Paper preserves these comments across saves
        c.options().setHeader(
            listOf(
                "E-WorldBorder configuration.",
                "Most values are managed in-game via /wb commands rather than edited by hand.",
                "Per-world borders live under the \"worlds\" section.",
            ),
        )
        c.setComments(
            "message",
            listOf(
                "Message shown to players knocked back from the border.",
                "Supports MiniMessage tags (e.g. <red>, <gradient:#ff5555:#ffff55>, <hover>, <click>)",
                "or legacy '&' colour codes for backwards compatibility.",
            ),
        )
        c.setComments("knock-back-dist", listOf("How far inside the border to move players. 0 disables all border enforcement."))
        c.setComments(
            "vanilla-border",
            listOf(
                "Mirror the border onto Minecraft's built-in (client-side) world border so players see the red wall.",
                "Square-only, so round/rectangular borders show as a square approximation; damage is disabled (visual).",
            ),
        )
        c.setComments("bstats-plugin-id", listOf("Set to your bStats plugin id (https://bstats.org) to enable anonymous metrics. 0 = disabled."))
        c.setComments("timer-delay-ticks", listOf("How often (in ticks) the plugin checks player positions against borders."))
